...
Sample usage:
python jsonl-hashtag-cooccur.py sample/sample-tweets-500.jsonl
"""
import sys, fileinput, itertools, io
...
def main():
				...
				if "entities" in tweet:
					if "hashtags" in tweet["entities"] and len(tweet["entities"]["hashtags"]) > 0:
						for tag in tweet["entities"]["hashtags"]:
							tweet_tags.append( "#" + tag["text"].lower().strip() )
				# process the pairs
				if len(tweet_tags) > 1:
					num_multiple += 1
	...
	fout = io.open( options.out_path, "w", encoding="utf-8", errors="ignore" )
